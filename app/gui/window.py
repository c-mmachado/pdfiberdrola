...
from app.model.pdfs import PDFLTMatchResult
...
class Window(QMainWindow, Ui_MainWindow):
    ...
    def _load_memento(self) -> None:
        if is_valid_file(self._MEMENTO_FILE):
            with open(self._MEMENTO_FILE, "r") as f:
                ...
                if memento:
                    self.input_files = [
                        make_path(f.strip()) for f in memento.get("input_files", [])
                    ]
    ...
    def browse_input_files(self) -> None:
        ...
        fnames: List[str] = self._open_file_dialog(
            caption="Selecciona archivos a procesar",
    ...
    def browse_input_dir(self) -> None:
        ...
        fnames: List[str] = self._open_file_dialog(
            caption="Selecciona una carpeta a procesar",
    ...
    def _resolve_input_files(self, fnames: List[str]) -> None:
        ...
        if fnames and TypeUtils.is_iterable(fnames):
            ...
            if len(self.input_files) > 1:
                fname = ", ".join(
                    [f'"{os.path.basename(f)}"' for f in self.input_files]
                )
            else:
                    ...
                    if len(file_names) > 0:
                        [
                            self.listWidget.addItem(
                                QListWidgetItem(self.pdfIcon, str(f))
                            )
                            for f in file_names
                        ]
                        has_items = True
                        ...
                        self.listWidget.addItem(
                            QListWidgetItem(
                                "No se encontraron archivos PDF en el directorio seleccionado."
                            )
                        )
                elif fname.endswith(".pdf"):
                    self.listWidget.addItem(
                        QListWidgetItem(self.pdfIcon, make_path(fname))
                    )
                    has_items = True
                else:
                    self.listWidget.addItem(
                        QListWidgetItem(
                            "Archivo no válido. Por favor, seleccione un archivo PDF."
                        )
                    )
    ...
    def browse_out_dir(self) -> None:
        ...
        fnames: List[str] = self._open_file_dialog(
            caption="Selecciona una carpeta de destino",
            file_mode=QFileDialog.FileMode.Directory,
            directory=directory,
        )
    ...
    def browse_template(self) -> None:
        ...
        fnames: List[str] = self._open_file_dialog(
            caption="Selecciona un archivo Excel",
    ...
    def process(self) -> None:
        ...
        items: List[QListWidgetItem] = [
            self.listWidget.item(x) for x in range(self.listWidget.count())
        ]
        pdf_paths: List[str] = []
        for item in items:
        ...
        work: Generator[Tuple[int, int, PDFLTMatchResult]] = parse_pdfs(
            pdfs_path=pdf_paths,
            out_dir=out_dir,
            split=split,
        ...
        while True:
            try:
                res: Tuple[int, int, PDFLTMatchResult] | Exception = next(work)
        ...
        self.label_4.setText(
            f"Proceso completado. Éxito: {len(items) - error_count} / Error: {error_count}"
        )
